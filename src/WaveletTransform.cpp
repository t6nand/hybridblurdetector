...
pair<int, float> WaveletTransform::calculateWaveletBlurExtent(Mat imageMat) {
	...
	for(int i = 0; i<m; i++)
	{
		for(int j = 0; j<n; j++)
		{
			if( (Emax1.at<float>(i,j)>tlzz_threshold) || (Emax2.at<float>(i,j)>tlzz_threshold) || (Emax3.at<float>(i,j)>tlzz_threshold))
			{
				++Nedge;
	...
	for(int i = 0; i<m; i++)
	{
		for(int j = 0; j<n; j++)
		{
			if((Eedge_Gstep_Roof.at<float>(i,j) > 0.1) && (Emax1.at<float>(i,j) < tlzz_threshold))
			{
				++Nbrg;
			}
		}
    ...
    if(Nbrg == 0 && Nrg == 0) {
       BlurExtent = -1.0f; // Denotes Both Nbrg & Nrg i.e. All total and blurred Roof-Structure and Gstep-Structure edge points were 0 in image.
    } else {
