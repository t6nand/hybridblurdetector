...
void BlockMetricsCalculatorIIR::calculate_blockwise_merits(ImageData& image) {
    ...
    for (int blocky = 0; blocky < image.imageHeightInBlocks; blocky++)
    {
        ...
        {
            cv::Rect blockRoi(blockx * BLOCK_WIDTH, blocky * BLOCK_HEIGHT, BLOCK_WIDTH, BLOCK_HEIGHT);
            calculate_block_sharpness_values(image, blockRoi, &image.blocks[blockIndex]);
            blockIndex++;
        }
    }
}

void BlockMetricsCalculatorIIR::calculate_block_sharpness_values(ImageData& image, cv::Rect& blockRoi, BlockMetrics *block)
{
    float pixels[((BLOCK_WIDTH > BLOCK_HEIGHT) ? BLOCK_WIDTH : BLOCK_HEIGHT) + FILTER_PAD] = { 0 };

    // Extract the block to process and create its transpose for processing columns
...
void BlockMetricsCalculatorIIR::calculate_block_reflectivity(BlockMetrics *block)
{
...
float BlockMetricsCalculatorIIR::sharpness(float *m, int n)
{
    ...
    for (int x = 0; x < n; x++) {
        ...
        // Integration of High frequency features energy till the next pixelin the block
