...
void RuleOfThird::rebuild_thirds_matrix() {
    ...
    for (int i = 0; i < h; i++)
        for (int j = 0; j < w; j++)
            this->weight[j + i*w] = 1.0 - min_manhattan_distance(j + 0.5, i + 0.5, power_points, 4, w, h);
}

BlockMetrics::BlockMetrics() : sharpness(0.0), reflectivity(0.0), brightness(0.0), mean_saturation(0.0),
...
ImageData::ImageData(char* fileName) : blocks(NULL), hsiChannels(3) {
    ...
    // Compute distance of center of each block with nearest Rule of thirds Intersection Points
    thirds = new RuleOfThird(imageWidthInBlocks, imageHeightInBlocks);

    // Initialize image to process to be of size in multiple of BLOCK_WIDTH and BLOCK_HEIGHT
    cv::Rect roi(0, 0, BLOCK_WIDTH * imageWidthInBlocks, BLOCK_HEIGHT * imageHeightInBlocks);
...
void ImageData::calculateBlockMeans() {
    ...
    cv::inRange(hsiChannels.at(0), cv::Scalar(SKY_HUE_LOW), cv::Scalar(SKY_HUE_HIGH), hueSkyLikeMat);
    ...
    for (int y = 0; y < image.rows; y += BLOCK_HEIGHT)
    {
        for (int x = 0; x < image.cols; x += BLOCK_WIDTH)
        {
            ...
            // Compute count of pixels in range (SKY_HUE_LOW, SKY_HUE_HIGH) in the block
