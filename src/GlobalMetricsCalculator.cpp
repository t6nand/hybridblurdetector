...
{
    double composition_sum, mean_bright_sharp, mean_bright_blur, mean_sat_sharp, mean_sat_blur;
    double *sorted_sharpness;
    int n, sharp_count;

    RuleOfThird* thirds = image.thirds;

    sorted_sharpness = (double*)malloc(sizeof(double)* image.imageWidthInBlocks *image.imageHeightInBlocks);
    composition_sum = mean_bright_sharp = mean_bright_blur = mean_sat_sharp = mean_sat_blur = 0.0;
    n = sharp_count = 0;

    for (int i = 0; i < image.imageWidthInBlocks*image.imageHeightInBlocks; i++) {
        // Ignore blocks of Sky - this wasn't used in testing due to bug in original repo.
    ...
    if (n % 2 == 0)
        image.merit.median_sharpness = (sorted_sharpness[n / 2] + sorted_sharpness[n / 2 - 1]) / 2.0f;
    else
        image.merit.median_sharpness = sorted_sharpness[n / 2];
    free(sorted_sharpness);

    image.merit.composition = composition_sum / n;
    image.merit.brightness_idx = (double) (mean_bright_sharp/(sharp_count + EPSILON_F) - mean_bright_blur/((n - sharp_count) + EPSILON_F));
    image.merit.saturation_idx = (double) (mean_sat_sharp/(sharp_count + EPSILON_F) - mean_sat_blur/((n - sharp_count) + EPSILON_F));
